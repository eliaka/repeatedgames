...
def act_gpt4(text, i, prev, prev2, prev3):
...
player_1 = [act_gpt4]
player_2 = [act_false_defector, act_grim_trigger, act_tft, act_tftt, act_stft, act_rtft, act_htft, act_np10, act_np20]
...
question = "You are playing a game repeatedly with another player. In this game, you can choose between Option J and Option F.\n"\
        f"You will play an indefinite number of rounds in total with the same player. There is a 60% probability that the game will end after each round.\n"\
...
df.to_csv('data/pd_gpt4_60p.csv', index=False)