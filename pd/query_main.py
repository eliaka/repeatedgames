...
player_list = [act_llama2, act_claude2, act_gpt4, act_gpt35, act_gpt3, act_defect_once, act_j, act_f]
...
for player_1, player_2 in itertools.product(player_list, player_list):
    ...
    for i in range(1, num_interactions+1, 1):
        ...
        if player_1.__name__ == "act_claude1" or player_1.__name__ == "act_claude2":
            answer_player1 = player_1(question + conversation_history_for_player1 + f"\nYou are currently playing round {i}.\n Which Option do you choose, Option " + choice_options[qorder_indices[0]] + " or Option " + choice_options[qorder_indices[1]] + "?", "Option")
        else:
        ...
        if player_2.__name__ == "act_claude1" or player_2.__name__ == "act_claude2":
            answer_player2 = player_2(question + conversation_history_for_player2 + f"\nYou are currently playing round {i}.\n Which Option do you choose, Option " + choice_options[qorder_indices[0]] + " or Option " + choice_options[qorder_indices[1]] + "?", "Option")
        else: 
            answer_player2 = player_2(question + conversation_history_for_player2 + f"\nYou are currently playing round {i}.\nQ: Which Option do you choose, Option " + choice_options[qorder_indices[0]] + " or Option " + choice_options[qorder_indices[1]] + "?\nA: Option", i-1)
        ...
        row = [i, player_1.__name__, player_2.__name__, answer_player1, answer_player2, round_points_for_player1, round_points_for_player2, points_for_player1, points_for_player2, qorder_indices[0], qorder_indices[1]]
        data.append(row)

    scores.append((points_for_player1, points_for_player2))

df = pd.DataFrame(data, columns=['round', 'player1', 'player2', 'answer1', 'answer2', 'points1', 'points2', 'total1', 'total2', 'option1', 'option2'])
df.to_csv('experiment_pd.csv', index=False)